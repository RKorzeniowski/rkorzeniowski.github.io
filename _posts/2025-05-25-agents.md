...
Furthermore it would be basically impossible to transfer quickly to a different game that structured differently but with the setup we are going to build it should only require updating a few variables. 
...
You can find all the code for agentic workflow [here](https://github.com/RKorzeniowski/BoardMaster/tree/main) and as a bonus vibe coded front end [here](https://github.com/RKorzeniowski/BoardMasterFrontEnd).
...
Getting this power of generative AI seems like something that in positive scenario could empower small groups of developers to build complete and more emersive than before experiences without it being labor intensive. The assets would be custom made by AI so it would not look like cheap junk games that reuse Unity like assets that are always a little bit off which makes the whole experience a second hand taste and discourages the usage. Creativity and high level thinking skills of artists even with limited level of technical skills when enabled could experience a renaissance.

All and all it's hard to predict the impact of tooling like that on a society as a whole. Things one would consider comparatively "simple" like a printing press had a profound cumulative effect on society. Even though humans had mechanisms for shareing knowledge and experience e.g. in person tutoring or hand written books, those were only available to the elites, due to thier high cost. When this shortcut to obtaining expert knowledge in months instead of decades for fraction of a cost became available to the masses suddenly bigger pool of talented individuals could study and further the development. Not only that but incentives created by availability of books boosted literacy rates and enabled more efficent long distance flow of information which is critical for invention. Second order effects of agents could be even bigger than the obvious things I mentioned before. 
